...
class Task(models.Model):
    ...
    def onchange_readings(self):
        ...
        if not self.x_serial_number_id and (self.x_black_copies or self.x_color_copies):
            raise UserError(_('You cannot enter Meter Readings without a Serial Number!.\nPlease enter a Serial Number first and try again'))
            return
    ...
    x_serial_number_street = fields.Char()
    x_serial_number_street2 = fields.Char()
    ...
    x_rental_group = fields.Char(related='x_serial_number_id.x_subscription_id.x_rental_group_id.name',string="Rental Group")
