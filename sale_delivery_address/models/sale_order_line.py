...
class SaleOrderLine(models.Model):
    ...
    delivery_addr_text = fields.Char(string="Detailed Address")
    ...
    @api.onchange('delivery_addr_id')
    def _onchange_branch_id(self):

        delv_detail = ''
        if self.delivery_addr_id.street:
            delv_detail = self.delivery_addr_id.street
        if self.delivery_addr_id.street2:
            delv_detail += self.delivery_addr_id.street2
        if self.delivery_addr_id.city:
            delv_detail += self.delivery_addr_id.city

        self.delivery_addr_text = delv_detail
